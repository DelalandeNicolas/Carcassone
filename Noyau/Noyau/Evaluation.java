...
public class Evaluation {
	...
	public ArrayList<Tuile> evalConstruction(){
		...
		while(!fini && !impossible){
			...
			if(!frontiere.isEmpty()){
				if(fils.isEmpty()){ // Si il y a encore des Evaluation dans frontiere mais aucun fils g�n�r�, alors la construction est imcompl�te
					impossible=true;
				} else { // ajoute les fils dans frontiere et dejaVus, retire le p�re de frontiere
		...
		if(!impossible){ // La boucle suivante met les Tuile des Evaluation de dejaVus dans construction, en �liminant les tuiles en double parmis les Evaluation 	
	...
	public void ajoutEtMaj (ArrayList<Evaluation>dejaVus, ArrayList<Evaluation>frontiere, ArrayList<Evaluation>fils){
		...
		for (int i = 0; i < fils.size(); i++) { // Pour chaque Evaluation dans fils...
			...
			if(!trouve){ // Si il n'y �tait pas on l'ajoute a dejaVus et frontiere
	...
	public ArrayList<Evaluation> genereFils(){
		...
		if(position==0){ // On s'int�resse a la caract�ristique au Nord de la Tuile.
			...
			if(connex[3][0]){
				// inutile pour l'�valuation des construction mais important pour rep�rer les pions pr�sent sur une m�me construction
	...
	// Renvoie la liste du(ou des) joueur(s) ayant la majorit� des pions sur une construction achev�. Et retire les pions.
		...
		for(int i=0;i<evalPosePion.size();i++){
			if(evalPosePion.get(i).getT().getPionPlace()!=null){ //Si il y a un pion sur cette tuile ...
				if(evalPosePion.get(i).t.getPionPlace().getPositionSurTuile()==evalPosePion.get(i).getPosition()){ // Pour ne pas comptabiliser un pion qui serait sur la tuile mais pas dans la construction qui nous int�resse
					nbpion[evalPosePion.get(i).getT().getPionPlace().getProprio().getIdentifiant()]++; // +1 dans la case du tableau ayant le m�me num�ro que l'id du joueur.
